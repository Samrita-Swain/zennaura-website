...
interface DatePickerInputType extends Omit<ReactDatePickerProps, 'icon'> {
  label: string;
  icon?: string | React.ReactElement | undefined; // Updated type
  containerClass?: string;
}

export default function DatePickerInput({
  label,
  icon = undefined, // Updated default value
  containerClass,
  ...props
}: DatePickerInputType) {
  const [state, setState] = useState(false);
