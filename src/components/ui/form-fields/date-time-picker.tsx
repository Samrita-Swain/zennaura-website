...
export interface DateTimeProps<selectsRange extends boolean | undefined>
  ...
  onChange(
    date: selectsRange extends false | undefined
      ? Date | null
      : [Date | null, Date | null],
    event: React.SyntheticEvent<HTMLElement> | undefined // Fixed any to HTMLElement
  ): void;
  /** Whether range selecting is enabled */
  selectsRange?: selectsRange;
  /** Pass input props to style input */
  startIcon?: React.ReactNode;
  startIconClassName?: string;
  endIcon?: React.ReactNode; // Optional, use if needed
  label?: string;
  error?: string;
  inputClassName?: string;
  labelClassName?: string;
...
const DateTime = ({
  ...
  startIcon,
  startIconClassName,
  // endIcon, // Removed unused variable if not used
  error,
  inputClassName,
  labelClassName,
  className,
