...
export interface PhoneNumberProps
  ...
  helperText?: React.ReactNode;
  className?: string;
  enableSearch?: boolean; // Add enableSearch to the interface
}
...
const PhoneNumber = ({
  ...
  return (
    <div className={clsx('aegon-phone-number', className)}>
      {label && (
        <label className={clsx('block', labelClasses.size[size], labelClassName)}>
          {label}
        </label>
      )}
      <div className="group/phone-number relative">
        <PhoneInput
          ...
          buttonClass={clsx(
            buttonClasses.base,
            buttonClasses.size[size],
            // @ts-expect-error: Ignore error due to potential incompatibility with the disabled or readOnly properties
            props.inputProps?.disabled && 'pointer-events-none',
            // @ts-expect-error: Ignore error due to potential incompatibility with the disabled or readOnly properties
            props.inputProps?.readOnly && 'pointer-events-none',
            buttonClassName
